...
def clean(s):
    ...
    # s = re.sub(r'(\/.*?\.[\S:]+)', ' ', s)
    s = re.sub('\]|\[|\)|\(|\=|\,|\;', ' ', s)#移除特定括号和分隔符
    s = " ".join([word.lower() if word.isupper() else word for word in s.strip().split()])#将全大写的单词转换为小写，保持其他单词不变，然后用空格连接所有单词。
    s = re.sub('([A-Z][a-z]+)', r' \1', re.sub('([A-Z]+)', r' \1', s))#使用正则表达式识别并分割驼峰命名的单词（如 "CamelCase" 变为 "Camel Case"）。
    s = " ".join([word for word in s.split() if not bool(re.search(r'\d', word))])#过滤掉所有包含数字的单词，只保留纯字母的单词。
    trantab = str.maketrans(dict.fromkeys(list(string.punctuation)))#使用 string.punctuation 创建一个翻译表，然后使用 translate 方法移除所有标点符号。
    content = s.translate(trantab)
    s = " ".join([word.lower().strip() for word in content.strip().split()])#将所有单词转换为小写，去除前后空格，并用空格连接
    return s


def performance_injection(sequence):#用于在日志序列中人为地创建性能相关的异常，以便测试和训练异常检测模型。
    ti = sequence[1]
    i = 0
    cnt = 5
    insert_idx = dict()
...
def load_HDFS(log_file, label_file=None, train_ratio=0.5, window='session',
    ...
    for k, v in data_dict.items():
        seq = [x[0] for x in v]#一个blockid下对应的所有eventsequence
        rt = [x[1] for x in v]
